...
class MockEpisodePlayerTest {
    ...
    @Test
    fun whenPlay_incrementsByPlaySpeed() =
    ...
    @Test
    fun whenPlayDone_playerAutoPlaysNextEpisode() =
        runTest(testDispatcher) {
    ...
    @Test
    fun whenNext_queueIsNotEmpty_autoPlaysNextEpisode() =
        runTest(testDispatcher) {
            val duration = Duration.ofSeconds(60)
            val currEpisode =
                PlayerEpisode(
    ...
    @Test
    fun whenPlayListOfEpisodes_playerAutoPlaysNextEpisode() =
        runTest(testDispatcher) {
            val duration = Duration.ofSeconds(60)
            val currEpisode =
                PlayerEpisode(
            ...
            val firstEpisodeFromList =
                PlayerEpisode(
                    uri = "firstEpisodeFromList",
                    duration = duration,
                )
            val secondEpisodeFromList =
                PlayerEpisode(
                    uri = "secondEpisodeFromList",
                    duration = duration,
                )
            val episodeListToBeAddedToTheQueue: List<PlayerEpisode> =
                listOf(
    ...
    @Test
    fun whenAddToQueue_queueIsNotEmpty() =
        runTest(testDispatcher) {
            testEpisodes.forEach { mockEpisodePlayer.addToQueue(it) }
    ...
    @Test
    fun whenNext_queueIsNotEmpty_removeFromQueue() =
        runTest(testDispatcher) {
            mockEpisodePlayer.currentEpisode =
                PlayerEpisode(
                    uri = "currentEpisode",
                    duration = Duration.ofSeconds(60),
                )
    ...
    @Test
    fun whenNext_queueIsNotEmpty_notRemovedFromQueue() =
        runTest(testDispatcher) {
            mockEpisodePlayer.currentEpisode =
                PlayerEpisode(
                    uri = "currentEpisode",
    ...
    @Test
    fun whenPrevious_queueIsEmpty_resetSameEpisode() =
        runTest(testDispatcher) {
            ...
            assertEquals(
                0,
                mockEpisodePlayer.playerState.value.timeElapsed
                    .toMillis(),
            )
