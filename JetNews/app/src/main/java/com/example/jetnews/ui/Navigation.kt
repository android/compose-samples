...
private object Saver {
    ...
    fun Screen.toBundle(): Bundle {
        ...
        if (current is Article) {
            retval.putString(SIS_ARG, current.postId)
        }
        return retval
    }
    ...
    fun Bundle?.toScreen(): Screen {
        ...
        return when (getString(SIS_NAME)) {
            ...
            ARTICLE.name -> {
                val postId = getString(SIS_ARG) ?: return Home // jump home if we fail to parse
                Article(postId)
            }
...
class NavigationViewModel(private val savedStateHandle: SavedStateHandle) : ViewModel() {
    ...
    init {
        with(Saver) {
            val SIS_SCREEN = "sis_screen"

            // load the saved screen (if any)
            currentScreen = savedStateHandle.get<Bundle>(SIS_SCREEN).toScreen()

            // save the screen to a bundle when `savedState` is called on savedStateHandler
            savedStateHandle.setSavedStateProvider(SIS_SCREEN) { currentScreen.toBundle() }
        }
    }
